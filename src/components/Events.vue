        ...
        <div class="page">
            <div class="eventsContent">
                <div class="nHeader"
                    ...
                    <h2>{{ lang == 'ar' ? 'اﻟﻔﻌﺎﻟﻴﺎت' : 'Events' }}</h2>
                    <p>{{ lang == 'ar' ? 'ﺗﺎﺑﻊ ﻣﻌﻨﺎ اﺧﺮ اﻻﺣﺪاث واﻟﻔﺎﻋﻠﻴﺎت اول ﺑﺄول' : 'Follow up on our latest events' }}</p>
                </div>
                <label class="loading" v-if="status == 'none'">
                  {{ lang == 'ar' ? 'عفواً لا توجد فاعليات' : 'Sorry, not events are available' }}
                </label>
                <label class="loading" v-if="status == 'search' && status != 'none'">
                    <!-- <font-awesome-icon class="spinner" icon="spinner" />&ensp; -->
                    <div class="lds-ring"><div></div><div></div><div></div><div></div></div>
                    {{ lang == 'ar' ? 'جاري تحميل الفاعليات..' : 'Loading events..' }}
                </label>
                <ul v-if="status == 'available'"
                    ...
                    <li v-for="e in events">
                        <div class="nCard">
                            <router-link :to="{ name: 'EventPage', params: {id: e.id} }">
                              <div class="nImg">
                                  <img :src="'https://api.alreaaiaa.com/' + e.image" @error="imgUrlAlt">
                              </div>
                              <div class="nContent">
                                  <h3>{{ lang == 'ar' ? e.title_ar : e.title_en }}</h3>
                                  <div>
                                    <font-awesome-icon icon="calendar"/>
                                    <label>{{ e.start_date }} - {{ e.end_date }}</label>
                                  </div>
                                  <div>
                                    <font-awesome-icon icon="clock"/>
                                    <label>{{ e.from_time }} - {{ e.to_time }}</label>
                                  </div>
                                  <div>
                                    <font-awesome-icon icon="map-marker-alt"/>
                                    <label>{{ lang == 'ar' ? e.address_ar : e.address_en }}</label>
                                  </div>
                              </div>
                            </router-link>
                        </div>
                ...
                <div v-if="pages.current < pages.total && events.length > 0" class="formCtrls">
                    <a @click="getEvents" v-if="!loadingMore" class="btnSuccess">{{ lang == 'ar' ? 'ﻋﺮض اﻟﻤﺰﻳﺪ ..' : 'Load more..' }}</a>
...
//const API_URL = "http://alreaaiaa.net/api/public/api/"
  ...
  methods: {
    getEvents() {
      ...
      axios.get(API_URL + "events?page="+Number(app.pages.current + 1)).then(function(response) {
        let data = response.data.data
        for(let i = 0; i < data.length; i++) {
          app.events.push(data[i])
        }
