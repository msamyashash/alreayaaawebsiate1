                    ...
                    <div class="content">
                        ...
                        <div class="contFilters">
                            ...
                            <ul data-aos="fade-up"
                                ...
                                <li v-for="spec in specialists" v-if="spec.companies_count != 0 && !specialistsLoading">
                                    <input type="radio" name="specialist" v-bind="{ 'id': 'specialist-'+spec.id, 'value': spec.id, 'checked': spec.id == cSpecialist ? true : false }">
                                    <label @click="setCSpecialists(spec.id)" v-bind="{ 'for': 'specialist-'+spec.id }">{{ lang == 'ar' ? spec.title_ar : spec.title_en }} <span>{{ spec.companies_count }}</span></label>
                                </li>
                            </ul>
                        </div>
                        <div class="results">
                            ...
                            <ul v-if="companies.length > 0 && companies != 'search' && companies != 'none'">
                                <li v-for="company of companies">
                                    <div class="liImg">
                                        <router-link :to="{ name: 'CompanyVisitor', params: {id: company.id} }">
  ...
  methods: {
    ...
    getCompanies() {
     ...
     if (app.country_id != "") {
      ...
      axios.post(API_URL + "web_search_by_all/"+app.lang,rData).then(function(response) {
