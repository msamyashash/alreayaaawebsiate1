    ...
    <div id="EventPage">
        <div class="page">
            ...
            <label class="loading" v-if="status == 'none'">
              {{ lang == 'ar' ? 'عفواً لا توجد تفاصيل للفاعلية' : 'Sorry, no details available' }}
            </label>
            <label class="loading" v-if="status == 'search' && status != 'none'">
                <!-- <font-awesome-icon class="spinner" icon="spinner" />&ensp; -->
                <div class="lds-ring"><div></div><div></div><div></div><div></div></div>
                {{ lang == 'ar' ? 'جاري تحميل الفاعلية..' : 'Loading details..' }}
            </label>
            <div class="eventContainer" v-if="status == 'available'">
                <div class="eventContent">
                    ...
                    data-aos-anchor-placement="top-center">
                        <img :src="'https://api.alreaaiaa.com/' + eDetails.image" @error="imgUrlAlt">
                    </div>
                    <div class="eDetails"
                    data-aos="fade-up"
                    data-aos-delay="200"
                    ...
                    data-aos-easing="ease-in-out"
                    data-aos-anchor-placement="top-center">
                        <h2>{{ lang == 'ar' ? eDetails.title_ar : eDetails.title_en }}</h2>
                        <div class="eInfo">
                            <div>
                              <font-awesome-icon icon="calendar"/>
                              <label>{{ eDetails.start_date }} - {{ eDetails.end_date }}</label>
                            </div>
                            <div>
                              <font-awesome-icon icon="clock"/>
                              <label>{{ eDetails.from_time }} - {{ eDetails.to_time }}</label>
                            </div>
                            <div>
                                <font-awesome-icon icon="map-marker-alt"/>
                                <label>{{ lang == 'ar' ? eDetails.address_ar : eDetails.address_en }}</label>
                            </div>
                        </div>
                        <p>{{ lang == 'ar' ? eDetails.discription_ar : eDetails.discription_en }}</p>
                      </div>
...
//const API_URL = "http://alreaaiaa.net/api/public/api/";
  ...
  methods: {
    getEvent() {
      let app = this;
      app.status = "search";
      axios.post(API_URL + "get_det_events", { 'event_id': app.id }).then(function(response) {
        app.eDetails = response.data[0];
        app.images = response.data[0].media;
        if (app.eDetails.length == 0) {
          app.status = "none";
        } else {
