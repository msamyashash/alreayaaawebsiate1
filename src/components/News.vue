        ...
        <div class="page">
            <div class="newsContent">
                <div class="nHeader"
                    ...
                    <h2>{{ lang == 'ar' ? 'اﻻﺧﺒﺎر' : 'News' }}</h2>
                    <p>{{ lang == 'ar' ? 'ﺗﺎﺑﻊ ﻣﻌﻨﺎ آﺧﺮ اﻟﺘﻄﻮرات' : 'Follow up with our latest news' }}</p>
                </div>
                <label class="loading" v-if="status == 'none'">
                  {{ lang == 'ar' ? 'عفواً لا توجد اخبار' : 'Sorry, no news available' }}
                </label>
                ...
                <ul v-if="status == 'available'"
                    ...
                    <li v-for="n in news">
                        <div class="nCard">
                            <div class="nImg">
                                <img :src="'https://api.alreaaiaa.com/' + n.image" @error="imgUrlAlt">
                            </div>
                            <div class="nContent">
                                <span class="nDate">{{ n.created_at }}</span>
                                <h3>{{ lang == 'ar' ? n.title_ar : n.title_en }}</h3>
                                <p>{{ lang == 'ar' ? n.description_ar : n.description_en }}</p>
                                <router-link :to="{ name: 'NewPage', params: { id: n.id } }">{{ lang == 'ar' ? 'اﻗﺮأ اﻟﻤﺰﻳﺪ' : 'Read more' }}</router-link>
                            </div>
                        </div>
                    </li>
                </ul>
                <div v-if="pages.current < pages.total && news.length > 0" class="formCtrls">
                    <a @click="getNews" v-if="!loadingMore" class="btnSuccess">{{ lang == 'ar' ? 'ﻋﺮض اﻟﻤﺰﻳﺪ ..' : 'Load more..' }}</a>
...
//const API_URL = "http://alreaaiaa.net/api/public/api/"
  ...
  methods: {
    getNews() {
      ...
      axios.get(API_URL + "news?page="+Number(app.pages.current + 1)).then(function(response) {
        // app.news = []
        let data = response.data.data
        for(let i = 0; i < data.length; i++) {
          app.news.push(data[i])
        }
