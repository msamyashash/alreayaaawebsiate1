        ...
        <div class="page">
            ...
            <div class="aContent">
                <div class="aHeader"
                    ...
                    <h2>{{ lang == 'ar' ? 'اﻟﺘﻜﺮﻳﻤﺎت' : 'Awards' }}</h2>
                    <p>{{ lang == 'ar' ? 'جوائز و تكريمات الرعاية الصحية' : 'Awards of Our Company' }}</p>
                </div>
                <label class="loading" v-if="status == 'none'">
                  {{ lang == 'ar' ? 'عفواً لا توجد تكريمات' : 'Sorry, there is now awards available' }}
                </label>
                <label class="loading" v-if="status == 'search' && status != 'none'">
                    <!-- <font-awesome-icon class="spinner" icon="spinner" />&ensp; -->
                    <div class="lds-ring"><div></div><div></div><div></div><div></div></div>
                    {{ lang == 'ar' ? 'جاري تحميل التكريمات..' : 'Loading awards..' }}
                </label>
                <ul v-if="status == 'available'"
                    ...
                    data-aos-easing="ease-in-out"
                    data-aos-anchor-placement="top-center">
                    <li v-for="(a, index) in awards">
                        <div v-if="lang == 'ar'" class="aCard" @click="viewAward(a.image,a.title_ar,index)">
                            <div class="aImg">
                        ...
                        <div v-if="lang == 'en'" class="aCard" @click="viewAward(a.image,a.title_en,index)">
                            <div class="aImg">
                                <img :src="'https://api.alreaaiaa.com/' + a.image" @error="imgUrlAlt">
                            </div>
                            <div class="aName">
                ...
                <div v-if="pages.current < pages.total && awards.length > 0" class="formCtrls">
                    <a @click="getAwards" v-if="!loadingMore" class="btnSuccess">{{ lang == 'ar' ? 'ﻋﺮض اﻟﻤﺰﻳﺪ ..' : 'Load more..' }}</a>
...
export default {
  ...
  methods: {
    viewAward(image, title, index) {
      this.images = this.awards[index].media
      if(this.awards[index].media.length > 0){
        if(image == this.awards[index].media[0].image){
          this.image = this.awards[index].media[0].image
        }else{
          this.image = image
        }
      } else {
    ...
    getAwards() {
      ...
      axios.get(API_URL + "honor?page="+Number(app.pages.current + 1)).then(function(response) {
        let data = response.data.data
        for(let i = 0; i < data.length; i++) {
          app.awards.push(data[i])
        }
        if (app.awards.length == 0) {
          app.status = "none"
        } else {
...
#Awards {
  .page {
    ...
    .aViewBlock {
      ...
      left: 0;
      opacity: 0;
      transition: 0.3s opacity ease-in, z-index .31s;
      &.active {
      z-index: 2000000;
        opacity: 1;
