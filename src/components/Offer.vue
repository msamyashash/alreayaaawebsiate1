<template>
    <div id="Offer">
        <div class="page">
            <div class="aViewBlock" v-bind:class="{ 'active': aView }">
                <div class="aFlex">
                    <div class="aModal">
                        <span class="modelClose" @click="closeAward">
                            <font-awesome-icon icon="times" />
                        </span>
                        <span v-if="images.length > 0" @click="nextImg" class="vBoxArrow nextImg">
                          <font-awesome-icon icon="chevron-right" />
                        </span>
                        <div class="mImg"
                          v-bind:style="{ background: 'url(https://api.alreaaiaa.com/' + image +') no-repeat center center / auto 100%' }"
                        >
                            <img style="opacity:0" :src="'https://api.alreaaiaa.com/' + image" @error="imgUrlAlt">
                        </div>
                        <span v-if="images.length > 0" @click="prevImg" class="vBoxArrow prevImg">
                          <font-awesome-icon icon="chevron-left" />
                        </span>
                    </div>
                </div>
            </div>
            <label class="loading" v-if="status == 'none'">
              {{ lang == 'ar' ? 'عفواً لا توجد تفاصيل للخبر' : 'Sorry, no details available' }}
            </label>
            <label class="loading" v-if="status == 'search' && status != 'none'">
                <!-- <font-awesome-icon class="spinner" icon="spinner" />&ensp; -->
                <div class="lds-ring"><div></div><div></div><div></div><div></div></div>
                {{ lang == 'ar' ? 'جاري تحميل الخبر..' : 'Loading details..' }}
            </label>
            <div class="newContainer" v-if="status == 'available'">
                <div class="newContent">
                    <div class="eImg"
                    data-aos="fade-down"
                    data-aos-duration="500"
                    data-aos-easing="ease-in-out"
                    data-aos-anchor-placement="top-center">
                        <img :src="'https://api.alreaaiaa.com/' + nDetails.spec_image" @error="imgUrlAlt">
                    </div>
                    <div class="eDetails"
                    data-aos="fade-up"
                    data-aos-delay="200"
                    data-aos-duration="500"
                    data-aos-easing="ease-in-out"
                    data-aos-anchor-placement="top-center">
                        <span class="nDate">{{ nDetails.created_at }}</span>
                        <h2>{{ lang == 'ar' ? nDetails.title_ar : nDetails.title_en }}</h2>
                        <div class="eInfo">
                            <div>
                              <font-awesome-icon style="padding: 0 12px 0 11px" icon="calendar"/>
                              <label>{{ lang == 'ar' ? 'من' : 'From' }} {{ nDetails.start_date }}</label>
                            </div>
                            <div>
                              <font-awesome-icon style="padding: 0 12px 0 11px;color: transparent" icon="calendar"/>
                              <label>{{ lang == 'ar' ? 'الى' : 'To' }} {{ nDetails.end_date }}</label>
                            </div>
                            <div style="margin-top: 15px">
                              <router-link :to="{ name: 'Company', params: { 'id': nDetails.health_company_id } }">
                                <font-awesome-icon icon="gift"/>
                                <label>{{ lang == 'ar' ? 'زيارة صفحة الشركة' : 'Visit company page' }}</label>
                              </router-link>
                            </div>
                        </div>
                        <p>{{ lang == 'ar' ? nDetails.description_ar : nDetails.description_en }}</p>
                    </div>
                    <div class="eMedia" v-if="images.length > 0">
                        <h2>{{ lang == 'ar' ? 'الصور' : 'Images' }}</h2>
                        <ul v-if="status == 'available'">
                          <li v-for="(m,index) in images">
                              <div class="aCard" @click="viewAward(m.image, index)">
                                  <div class="aImg">
                                      <img :src="'https://api.alreaaiaa.com/' + m.image" @error="imgUrlAlt">
                                  </div>
                              </div>
                          </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <Footer></Footer>
    </div>
</template>

<script>
    var Tawk_API=Tawk_API||{}, Tawk_LoadStart=new Date();
    (function(){
        var s1=document.createElement("script"),s0=document.getElementsByTagName("script")[0];
        s1.async=true;
        s1.src='https://embed.tawk.to/5ba88858c9abba579677d3d7/default';
        s1.charset='UTF-8';
        s1.setAttribute('crossorigin','*');
        s0.parentNode.insertBefore(s1,s0);
    })();
const axios = require("axios");
// const API_URL = "https://cors-anywhere.herokuapp.com/http://api.alreaaiaa.com/api/"
//const API_URL = "http://alreaaiaa.net/api/public/api/"
     const API_URL = "https://api.alreaaiaa.com/api/";
export default {
  name: "Offer",
  metaInfo: {
      title:localStorage.getItem('lang')=='en'?'Health Care':'الرعاية الصحية',
    },
  data() {
    return {
      aView: false,
      image: "",
      id: this.$route.params.id,
      nDetails: {},
      images: [],
      currentImg: 0,
      status: "search",
      lang: "ar"
    };
  },
  methods: {
    getNew() {
      let app = this;
      app.status = "search";
      axios
        .post(API_URL + "get_det_special_offers", { id: app.id })
        .then(function(response) {
          app.nDetails = response.data;
          app.images = response.data.media;
          if (app.nDetails.length == 0) {
            app.status = "none";
          } else {
            app.status = "available";
          }
        });
    },
    viewAward(image, index) {
      this.currentImg = index;
      this.image = image;
      this.aView = true;
    },
    closeAward() {
      return (this.aView = false);
    },
    nextImg() {
      let total = Number(this.images.length - 1);
      let curImg = this.currentImg;
      if (curImg < total) {
        this.currentImg = Number(curImg + 1);
        this.image = this.images[this.currentImg].image;
      } else {
        this.currentImg = 0;
        this.image = this.images[this.currentImg].image;
      }
    },
    prevImg() {
      let total = Number(this.images.length - 1);
      let curImg = this.currentImg;
      if (curImg >= 1) {
        this.currentImg = Number(curImg - 1);
        this.image = this.images[this.currentImg].image;
      } else {
        this.currentImg = total;
        this.image = this.images[this.currentImg].image;
      }
    },
    imgUrlAlt(event) {
      event.target.src =
        "data:image/png;base64,";
    }
  },
  created: function() {
    let app = this;
    app.lang = localStorage.getItem("lang");
    app.getNew();
  }
};
</script>
<style lang="scss">
#Offer {
  .page {
    width: 100%;
    min-height: calc(100vh - 165px);
    overflow: hidden;
    .eMedia {
      width: 100%;
      display: inline-block;
      text-align: center;
      padding: 40px 0;
      h2 {
        display: inline-block;
        width: 100%;
        color: #18b4bb;
        margin-top: 0;
        font-size: 150%;
      }
      ul {
        display: -ms-flexbox;
        -ms-flexbox-wrap: wrap;
        -ms-flexbox-direction: column;
        -webkit-flex-flow: row wrap;
        flex-flow: row wrap;
        display: -webkit-box;
        display: flex;
        list-style: none;
        padding: 0;
        width: 90%;
        margin: 0 auto;
        li {
          margin: 5px 10px;
          float: right;
          height: auto;
          width: 200px;
          -webkit-box-flex: auto;
          -ms-flex: auto;
          flex: auto;
          display: flex;
          justify-content: center;
          align-items: center;
          background: #f7f7f7;
          box-shadow: 1px 2px 8px 1px rgba(0, 0, 0, 0.25);
          transition: 0.3s all;
          &:hover {
            background: #e7e7e7;
            box-shadow: 1px 2px 12px 1px rgba(0, 0, 0, 0.32);
          }
          .aCard {
            display: block;
            width: 100%;
            .aImg {
              text-align: center;
              img {
                cursor: pointer;
                width: 100%;
                height: auto;
              }
            }
          }
        }
      }
    }
    .aViewBlock {
      position: fixed;
      z-index: -1;
      background: rgba(0, 0, 0, 0.5);
      height: 100vh;
      width: 100vw;
      top: 0;
      left: 0;
      opacity: 0;
      transition: 0.3s opacity ease-in, z-index 0.31s;
      &.active {
        z-index: 2000000;
        opacity: 1;
        transition: 0.3s opacity ease-out, z-index 0.01s;
      }
      .aFlex {
        height: 100%;
        width: 100%;
        display: flex;
        justify-content: center;
        align-items: center;
        .aModal {
          height: 600px;
          position: relative;
          width: 800px;
          overflow: hidden;
          max-width: 100%;
          max-height: 90%;
          background: #fff;
          border-radius: 15px;
          .modelClose {
            position: absolute;
            z-index: 10;
            font-size: 160%;
            color: #909090;
            padding: 5px 20px;
            cursor: pointer;
            &:hover {
              color: #606060;
            }
          }
          .vBoxArrow {
            position: absolute;
            float: right;
            z-index: 10;
            font-size: 300%;
            color: #909090;
            padding: 5px 20px;
            cursor: pointer;
            top: 50%;
            margin-top: -50px;
            &.nextImg {
              right: 0;
            }
            &.prevImg {
              left: 0;
            }
            &:hover {
              color: #606060;
            }
          }
          .mImg {
            text-align: center;
            background: #f7f7f7;
            height: 100%;
            overflow: hidden;
            display: flex;
            justify-content: center;
            align-items: center;
            transition: 0.3s background-image linear;
            img {
              height: auto;
              width: auto;
              max-width: 100%;
            }
          }
        }
      }
    }
    .newContainer {
      width: 1024px;
      max-width: 100%;
      margin: 0 auto;
      padding: 50px 0;
      display: table;
      .newContent {
        display: table-row;
        width: 100%;
        .eImg {
          width: calc(50% - 10px);
          padding-right: 10px;
          display: column;
          float: right;
          text-align: center;
          img {
            max-width: 100%;
          }
        }
        .eDetails {
          width: calc(50% - 50px);
          padding-right: 40px;
          padding-left: 10px;
          display: column;
          float: right;
          h2 {
            display: inline-block;
            width: 100%;
            color: #18b4bb;
            margin-top: 0;
            font-size: 150%;
          }
          .nDate {
            color: #909090;
          }

          .eInfo {
            display: inline-block;
            border-top: 1.5px #b7b7b7 solid;
            border-bottom: 1.5px #b7b7b7 solid;
            width: 100%;
            padding: 18px 0;
            color: #707070;
            a {
              width: 90%;
              display: inline-block;
              text-decoration: none;
              color: #18b4bb;
              font-weight: 700;
              cursor: pointer;
              svg {
                display: block;
              cursor: pointer;
                width: 40px;
                float: right;
                font-size: 115%;
                text-align: center;
                margin-top: 4px;
              }
              label {
                display: block;
              cursor: pointer;
                float: right;
                width: calc(100% - 40px);
              }
            }
          }
          p {
            white-space: pre-line;
            text-align: justify;
            color: #707070;
            font-size: 105%;
            font-weight: 700;
          }
        }
      }
    }
  }
}
@media screen and (max-width: 768px) {
  #Offer {
    .page {
      min-height: calc(100vh - 120px);
      .newContainer {
        display: inline-block;
        .newContent {
          display: inline-block;
          width: 100%;
          .eImg {
            width: calc(100% - 40px);
            padding: 0;
            margin: 0 20px;
            display: inline-block;
            img {
              min-width: 100%;
            }
          }
          .eDetails {
            width: calc(100% - 40px);
            padding: 0;
            margin: 0 20px;
            display: inline-block;
          }
        }
      }
    }
  }
}
</style>
